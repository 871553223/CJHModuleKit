...
@implementation CJHPaymentLoadingHUD
{
    CADisplayLink *_link;
...
+(CJHPaymentLoadingHUD*)showIn:(UIView*)view{
    [self hideIn:view];
    CJHPaymentLoadingHUD *hud = [[CJHPaymentLoadingHUD alloc] initWithFrame:view.bounds];
    [hud start];
    [view addSubview:hud];
    return hud;
}
...
+(CJHPaymentLoadingHUD *)hideIn:(UIView *)view{
    CJHPaymentLoadingHUD *hud = nil;
    for (CJHPaymentLoadingHUD *subView in view.subviews) {
        if ([subView isKindOfClass:[CJHPaymentLoadingHUD class]]) {
            [subView hide];
