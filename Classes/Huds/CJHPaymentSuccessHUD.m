...
@implementation CJHPaymentSuccessHUD {
...
+ (CJHPaymentSuccessHUD*)showIn:(UIView*)view {
    [self hideIn:view];
    CJHPaymentSuccessHUD *hud = [[CJHPaymentSuccessHUD alloc] initWithFrame:view.bounds];
    [hud start];
    [view addSubview:hud];
    return hud;
}
...
+ (CJHPaymentSuccessHUD *)hideIn:(UIView *)view {
    CJHPaymentSuccessHUD *hud = nil;
    for (CJHPaymentSuccessHUD *subView in view.subviews) {
        if ([subView isKindOfClass:[CJHPaymentSuccessHUD class]]) {
            [subView hide];
